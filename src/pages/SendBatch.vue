<template>
  <q-page>
    <q-toolbar v-if="showHeader" class="bg-accent text-warning">
      <q-btn flat size="sm" round icon="arrow_back_ios" to="/send" replace />
      <q-toolbar-title class="text-center text-subtitle1 text-bold">{{ $t('sendBatch.title') }}</q-toolbar-title>
      <q-btn flat round icon="more_vert" class="invisible" />
    </q-toolbar>
    <q-card flat class="bg-accent batch-send-card">
      <q-tab-panels v-model="mode" class="bg-accent" animated>
    ...
    <div v-if="receivePairs.length" class="outputs-card bg-transparent q-mt-lg">
      <div class="row justify-between items-center q-pa-sm bg-accent text-white">
        <div class="text-caption">
          {{$t('sendBatch.label.sum')}}:
          <b>{{sum}}</b> CKB
        </div>
      ...
      >
        ...
        <q-btn icon="delete_outline" round dense outline size="sm" @click="removePair(i)" />
      </div>
      <div class="q-pa-md">
        <q-btn
          color="primary"
