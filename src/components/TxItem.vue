<template>
  <div
    class="row q-pa-sm justify-between items-baseline"
    @click="!pending && $emit('click')"
  >
    <div class="column">
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      const amount = computed(() =>
        props.tx.amount.toString(AmountUnit.ckb, { commify: true, fixed: 2 })
      );
      const icon = computed(() =>
        tag.value === 'from'
          ? 'arrow_downward'
